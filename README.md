...
## 特徴
- ホーム画面（番号アイコン）から各機能へワンタップ遷移
- 本番日カウントダウン（11月第2日曜の暫定日を自動設定）
- 本番7日前を仕上げ期限として逆算（D-7までに完成する学習計画）
- 「連続満点でセクションクリア / ミスでそのセクション先頭へ戻る」反復ドリル
- 各ドリルで「要点チェック(教科書) → 3択回答 → 解説確認」の順で学習
- 問題セットをセクション分割して進行（目安5問単位）
- 5セクションクリアごとに小テストへ進むサイクル
- 問題はすべて3択（選択式）
...
6. 「小テスト」で10問の本番風チェック
7. 必要なら「問題と解説の登録」で、あなたの手持ち過去問の解説を蓄積
8. 「180分模試を開始」で本番形式トレーニング
